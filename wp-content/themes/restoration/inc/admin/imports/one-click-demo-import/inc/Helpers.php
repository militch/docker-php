...
class Helpers {
	...
	public static function write_to_file( $content, $file_path ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf(
					// Translators: %2$s : file path.
					__( 'An error occurred while writing file to your server! Tried to write a file to: %1$s%2$s.', 'restoration' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf(
					// Translators: %2$s : file path.
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %1$s%2$s.', 'restoration' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf(
					// Translators: %3$s : link
					__( 'This WordPress page does not have %1$sdirect%2$s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %3$s.', 'restoration' ),
					'<strong>',
					'</strong>',
		...
		$plugin_page_setup = apply_filters(
			...
			array(
				...
				'page_title'  => esc_html__( 'One Click Demo Import', 'restoration' ),
				'menu_title'  => esc_html__( 'Import Demo Data', 'restoration' ),
				'capability'  => 'import',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new \WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'restoration' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'restoration' )
			);
		}
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$attachment = array(
			...
			'post_title'     => apply_filters( 'pt-ocdi/attachment_prefix', esc_html__( 'One Click Demo Import - ', 'restoration' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf(
					...
					__( '%1$sYour user role isn\'t high enough. You don\'t have permission to import demo data.%2$s', 'restoration' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					// Translators: %s: error.
					__( 'Content file was not uploaded. Error: %s', 'restoration' ),
					$widget_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files', 'restoration' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					// Translators: %s: error.
					__( 'Widget file was not uploaded. Error: %s', 'restoration' ),
					$widget_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files', 'restoration' )
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					// Translators: %s: error.
					__( 'Customizer file was not uploaded. Error: %s', 'restoration' ),
		...
		if ( $redux_file_info && ! isset( $redux_file_info['error'] ) ) {
			if ( isset( $_POST['redux_option_name'] ) && empty( $_POST['redux_option_name'] ) ) {
				...
				self::log_error_and_send_ajax_response(
					esc_html__( 'Missing Redux option name! Please also enter the Redux option name!', 'restoration' ),
					$log_file_path,
					esc_html__( 'Upload files', 'restoration' )
				);
			}
			...
			$log_added = self::append_to_file(
				sprintf(
					// Translators: %s: error.
					__( 'Redux file was not uploaded. Error: %s', 'restoration' ),
		...
		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'restoration' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files', 'restoration' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		return PHP_EOL .
			sprintf(
				...
				__( 'Initial max execution time = %s', 'restoration' ),
				ini_get( 'max_execution_time' )
			) . PHP_EOL .
			sprintf(
				// Translators: %2%s: site url, others: demo file urls.
				__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s%1$sRedux files:%1$s%6$s', 'restoration' ),
				PHP_EOL,
				get_site_url(),
				empty( $selected_import_files['content'] ) ? esc_html__( 'not defined!', 'restoration' ) : $selected_import_files['content'],
				empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'restoration' ) : $selected_import_files['widgets'],
				empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'restoration' ) : $selected_import_files['customizer'],
				empty( $redux_file_string ) ? esc_html__( 'not defined!', 'restoration' ) : $redux_file_string
			);
	}
