...
//		$this->google_active   = $this->google_active();
			...
			if (Options::is_google_optimize_anti_flicker_active()) {
				add_action('wp_head', function () {
					...
					<script>(function(a,s,y,n,c,h,i,d,e){s.className+=' '+y;h.start=1*new Date;
						...
						})(window,document.documentElement,'async-hide','dataLayer',<?php esc_html_e(apply_filters('pmw_google_optimize_anti_flicker_timeout', 4000)); ?>,
		...
		if (true && $this->is_tiktok_eapi_active__premium_only()) {
		...
		if (Environment::get_instance()->is_woocommerce_active()) {
		...
		<table>
			...
			<?php foreach ($data['rows'] as $row) : ?>
				<tr>
					...
					<td class="pmw-ga4-attr-values"><?php esc_html_e(wc_format_decimal(floatval($row['metricValues'][0]['value']), 2)); ?></td>
			...
			<tr>
				<td><b>Total</b></td>
				<td class="pmw-ga4-attr-values"><b><?php esc_html_e(wc_format_decimal($total_value, 2)); ?></b></td>
				<td class="pmw-ga4-attr-values"><b>100%</b></td>
			</tr>
...
//			wc_get_logger()->debug('Invalid Options. Options not saved', ['source' => 'PMW']);
	...
	public function run_background_processes() {
		...
		if (true && Environment::get_instance()->is_woocommerce_active()) {
			...
			if (is_cart() || is_checkout()) {
				...
				if ($this->is_tiktok_eapi_active__premium_only()) {
	...
	public function woocommerce_after_cart_item_name( $cart_item, $cart_item_key ) {
		...
		<script>
			...
			window.wpmDataLayer.cartItemKeys['<?php echo esc_js($cart_item_key); ?>'] = <?php echo wp_json_encode($data, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?>;
		</script>
	...
	public function woocommerce_inject_product_data_on_product_page() {
		...
		if (Product::is_not_wc_product($product)) {
			...
			wc_get_logger()->debug('woocommerce_inject_product_data_on_product_page provided no product on a product page: .' . get_the_id(), ['source' => 'PMW']);
	...
	private function inject_data_layer() {
		...
		<script>
			...
			window.wpmDataLayer = Object.assign(window.wpmDataLayer, <?php echo wp_json_encode($this->get_data_for_data_layer(), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?>)
	...
	protected function get_google_pixel_data() {
		...
		if ($this->google->is_google_ads_active()) {
			$data['ads'] = [
				...
				'dynamic_remarketing'      => [
					...
					'send_events_with_parent_ids' => apply_filters('wpm_send_events_with_parent_ids', apply_filters_deprecated('wooptpm_send_events_with_parent_ids', [true], '1.13.0', 'wpm_send_events_with_parent_ids')),
				],
		...
		if ($this->google->is_google_analytics_active()) {
			$data['analytics'] = [
				...
				'ga4'       => [
					...
					'mp_active'      => ( $this->options_obj->google->analytics->ga4->api_secret && true ),
					'debug_mode'     => $this->google->is_ga4_debug_mode_active(),
				],
				'id_type'   => $this->google->get_ga_id_type(),
				'eec'       => true && $this->google->is_google_analytics_active(),
			];
		}
		...
		if (true) {
			$data['consent_mode'] = [
				...
				'ad_storage'         => $this->options_obj->shop->cookie_consent_mgmt->explicit_consent ? 'denied' : 'granted',
				'analytics_storage'  => $this->options_obj->shop->cookie_consent_mgmt->explicit_consent ? 'denied' : 'granted',
				'wait_for_update'    => 500,
	...
	protected function get_order_data( $order ) {
		...
		if ($order) {
			$data['order'] = [
				...
				'value_filtered'   => (float) Shop::pmw_get_order_total($order, true),
				'value_regular'    => (float) $order->get_total(),
...
//			error_log(print_r($order, true));
	...
	public function view_item_list_trigger_settings() {
		...
		$settings = apply_filters_deprecated('wooptpm_view_item_list_trigger_settings', [$settings], '1.13.0', 'wpm_view_item_list_trigger_settings');
...
//				wc_get_logger()->debug('ajax_pmw_get_product_ids received an invalid product', ['source' => 'PMW']);
	...
	public function wpm_front_end_scripts() {
		...
		if (true && apply_filters('wpm_experimental_inject_polyfill_io', false)) {
		...
		if (true) {
			...
			wp_enqueue_script(
				'wpm',
				WPM_PLUGIN_DIR_PATH . 'js/public/wpm-public__premium_only' . $this->get_preset_version() . '.min.js',
				$pmw_dependencies,
				WPM_CURRENT_VERSION,
			...
			if (Helpers::lazy_load_pmw__premium_only()) {
				wp_enqueue_script(
					'pmw-lazy',
					WPM_PLUGIN_DIR_PATH . 'js/public/pmw-lazy__premium_only.js',
					['wpm'],
					WPM_CURRENT_VERSION,
					$this->move_wpm_script_to_footer()
				);
			...
			if (Helpers::partytown__premium_only()) {
				...
				wp_enqueue_script(
					'partytown',
					WPM_PLUGIN_DIR_PATH . 'js/public/partytown/partytown.js',
					[],
					WPM_CURRENT_VERSION
				);
			}
...
//		$data['mini_cart']['track'] = apply_filters('wpm_track_mini_cart', $data['mini_cart']['track']);
		...
		$mini_cart_filter_deprecation_message
			= 'The filter has become obsolete since WPM now tracks cart item data using the browser cache and doesn\'t rely entirely on the server anymore.';
	...
	protected function is_tiktok_eapi_active__premium_only() {
		return $this->options_obj->tiktok->pixel_id && $this->options_obj->tiktok->eapi->token;
	}
}
