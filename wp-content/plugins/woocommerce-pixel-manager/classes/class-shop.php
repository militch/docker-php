...
class Shop {
	...
	public static function pmw_get_order_total( $order, $apply_multipliers = false ) {
		...
		return wc_format_decimal((float) $order_total, 2);
	}
	...
	public static function can_order_confirmation_be_processed( $order ) {
		...
		$conversion_prevention = apply_filters_deprecated('wooptpm_conversion_prevention', [$conversion_prevention, $order], '1.13.0', 'wpm_conversion_prevention');
...
//		error_log('parse url https://www.exampel.com : ' . parse_url('https://www.exampel.com', PHP_URL_HOST));
	...
	public static function can_clv_query_be_run( $billing_email ) {
		...
		if (!Environment::get_instance()->is_memory_limit_higher_than('100M')) {
			return false;
		}
	...
	public static function get_clv_value_filtered_by_billing_email( $billing_email ) {
		...
		foreach ($orders as $order) {
			$value += (float) self::pmw_get_order_total($order);
		}
	...
	public static function get_order_with_url_order_key() {
		...
		if (isset($_get['key']) || isset($_get['wcf-key'])) {
			...
			if (isset($_get['key'])) {
				$order_key = $_get['key'];  // for WooCommerce keys
			} else {
				$order_key = $_get['wcf-key']; // for CartFlows keys
			}
